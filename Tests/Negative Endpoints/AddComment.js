...
pm.test("Comment already exists", function () {
    let existingCommentId = Number(pm.environment.get("commentId"));
    let requestData = JSON.parse(pm.request.body.raw);
    if (requestData.id === existingCommentId) {
        pm.expect(pm.response.code).to.equal(409);
    }else{
        console.log("Comment doesn't use existing id")
    }
});
