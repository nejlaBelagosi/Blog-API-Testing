...
// Idempotency: Editing a deleted post returns 404 (if applicable)
...
pm.test("Should return 404 for non-existing post", function () {
    if (pm.environment.get('nonExistingPostId') && pm.request.url.path.includes(pm.environment.get('nonExistingPostId'))) {
        pm.expect(pm.response.code).to.eql(404);
        let json = {};
        try { json = pm.response.json(); } catch (e) { }
        pm.expect(hasErrorMessage(json)).to.be.true;
