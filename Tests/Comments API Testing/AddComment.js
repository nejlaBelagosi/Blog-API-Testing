...
pm.test("Check if API endpoint URL is correct", function(){
    pm.expect(pm.request.url.toString()).to.eql(`https://jsonplaceholder.typicode.com/comments`);
}); 
...
// Check additional fields (if expected)
pm.test("Response contains postId and email", function () {
